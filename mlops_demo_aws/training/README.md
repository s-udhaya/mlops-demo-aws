...
directory of the corresponding component, for example, model training notebooks in `mlops_demo_aws/training/notebooks`, 
batch inference notebook in `mlops_demo_aws/deployment/batch_inference/notebooks`.
...
Address TODOs in the recipe configs under `mlops_demo_aws/training/recipe.yaml`, `mlops_demo_aws/training/profiles/databricks-dev.yaml`,
and `mlops_demo_aws/training/profiles/local.yaml`, specifying configs such as the training dataset path(s) to use when developing
...
After configuring your recipe, you can iterate on and test ML code under ``mlops_demo_aws/training/steps``.
We expect most development to take place in the abovementioned YAML config files and
`mlops_demo_aws/training/steps/train.py` (model training logic).

## Iterating on ML code

### Deploy ML code and resources to dev workspace using Bundles

Refer to [Local development and dev workspace](../../mlops_demo_aws/databricks-resources/README.md#local-development-and-dev-workspace) 
...
* Run `databricks configure --profile mlops-demo-aws-dev --token --host <your-dev-workspace-url>`, passing the URL of your dev workspace.
...
* From within the root directory of the current project, use the [dbx sync](https://dbx.readthedocs.io/en/latest/guides/python/devloop/mixed/#using-dbx-sync-repo-for-local-to-repo-synchronization) tool to copy code files from your local machine into the Repo by running
  `dbx sync repo --profile mlops-demo-aws-dev --source . --dest-repo your-repo-name`, where `your-repo-name` should be the last segment of the full repo name (`/Repos/username/your-repo-name`)

#### Running code on Databricks
You can iterate on ML code by running the provided `mlops_demo_aws/training/notebooks/Train.py` notebook on Databricks using
[Repos](https://docs.databricks.com/repos/index.html). This notebook drives execution of
the ML code defined under ``mlops_demo_aws/training/steps``. You can use multiple browser tabs to edit
